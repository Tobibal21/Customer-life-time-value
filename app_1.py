...
# Function to prepare RFM scores
def prepare_rfm_scores(df):
    """Prepare RFM scores for the dataframe"""
    df_copy = df.copy()
    try:
        df_copy['recency_score'] = pd.qcut(df_copy['recency_days'], q=4, labels=['4', '3', '2', '1'], duplicates='drop')
        df_copy['frequency_score'] = pd.qcut(df_copy['n_transactions'], q=4, labels=['1', '2', '3', '4'], duplicates='drop')
        df_copy['monetary_score'] = pd.qcut(df_copy['total_spent'], q=4, labels=['1', '2', '3', '4'], duplicates='drop')
    except Exception as e:
        # Fallback to rank-based binning if qcut fails
        df_copy['recency_score'] = pd.qcut(df_copy['recency_days'].rank(method='first'), q=4, labels=['4', '3', '2', '1'])
        df_copy['frequency_score'] = pd.qcut(df_copy['n_transactions'].rank(method='first'), q=4, labels=['1', '2', '3', '4'])
        df_copy['monetary_score'] = pd.qcut(df_copy['total_spent'].rank(method='first'), q=4, labels=['1', '2', '3', '4'])
...
# Prepare RFM scores for all analyses
df = prepare_rfm_scores(df)
...
if analysis_type == "Overview":
    ...
    with col1:
        ...
        <div class="metric-card">
            <div class="metric-value">${df['clv'].mean():,.2f}</div>
    ...
    with col2:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">${df['avg_order_value'].mean():,.2f}</div>
    ...
    with col3:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{df['n_transactions'].mean():.1f}</div>
    ...
    with col4:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{df['churn_risk'].mean():.1f}%</div>
...
# Customer Segmentation
    ...
    # Segment definition (using existing RFM scores)
...
# RFM Analysis
    ...
    # RFM matrix - Now using the pre-calculated scores
    ...
    top_rfm_segments = df['rfm_segment'].value_counts().head(10)
    
    fig = px.bar(x=top_rfm_segments.values, y=top_rfm_segments.index, orientation='h',
                title="Top 10 RFM Segments",
                labels={'x': 'Number of Customers', 'y': 'RFM Segment'})
    st.plotly_chart(fig, use_container_width=True)
